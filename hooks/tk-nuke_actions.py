...
class NukeActions(HookBaseClass):
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        app.log_debug("Execute action called for action {}. "
                      "Parameters: {}. Data: {}".format(name, params, sg_publish_data))

        params = params or {}
        drag_and_dropped = params.get('drag_and_dropped', False)

        if name == "read_node":
            path = self.get_publish_path(sg_publish_data).replace(os.path.sep, "/")
            self._create_read_node(path)

        elif name == "script_import":
            path = self.get_publish_path(sg_publish_data).replace(os.path.sep, "/")
            if drag_and_dropped:
                self._import_dragged_script(path, sg_publish_data)
            else:
        ...
        # shotgundropper actions
        elif name == "drop_version":
            path = sg_publish_data.get('sg_path_to_frames') or sg_publish_data.get('sg_path_to_movie')
            if path is not None:
                self._create_read_node(path)

        elif name == "drop_playlist":
            sg_versions = self.get_playlist_versions(sg_publish_data['id'])
            for sg_version in sg_versions:
                self.execute_action('drop_version', {}, sg_version)
    ...
    def _import_dragged_script(self, path, sg_publish_data):
        """
        Import contents of the dropped file into the scene.
        We need this workaround because of a bug with the "nodePaste" function.
        for more information see:
         "https://support.foundry.com/hc/en-us/articles/360004001400?flash_digest=38eaf834442eeb47aace76849bce54c53675c3fc"
        ...
        def _paste_drop_nuke_file(text):
            nuke.nodePaste(text)

        QTimer.singleShot(0, lambda: _paste_drop_nuke_file(path))
    ...
    def _create_read_node(self, path):
        """
    ...
    def load_shot(self):
        import nuke
        versions = self.get_versions({'type': 'Shot', 'id': sg_entity['id']}, ignore=['omt'])

        for v in versions:
            nuke.createNode('Read',
                            'file {sg_path_to_frames} '
                            'first {sg_first_frame} last {sg_last_frame} '
                            'origfirst {sg_first_frame} origlast {sg_last_frame} '
                            'label {sg_version_type}'.format(**v))

    def get_versions(self, link, ignore=None):
        """
        ...
        """
        versions = self.parent.shotgun.find('Version',
                                            filters=[['entity', 'is', link]],
                                            fields=['code', 'sg_status_list', 'sg_version_type',
                                                    'sg_path_to_frames', 'sg_first_frame', 'sg_last_frame'])
        if not ignore:
            return versions
        return [v for v in versions if v['sg_status_list'] not in ignore]

    def get_playlist_versions(self, playlist_id):
        """
        :param int playlist_id: The ID of the playlist
        :returns: All Versions connected to the given playlist.
        :rtype: list[dict]
        """
        return self.parent.shotgun.find('Version',
                                        filters=[['playlists', 'is', {'type': 'Playlist', 'id': playlist_id}]],
                                        fields=['sg_path_to_frames', 'sg_path_to_movie', 'sg_first_frame',
                                                'sg_last_frame'])
